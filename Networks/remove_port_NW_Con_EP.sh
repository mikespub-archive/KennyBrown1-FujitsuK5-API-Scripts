...
NETCON_EP_ID=  ## Add network connector endpoint ID
PORT_ID=   ## Add network port ID

curl -s -X PUT $NETWORK/v2.0/network_connector_endpoints/$NETCON_EP_ID/disconnect -H "X-Auth-Token: $OS_AUTH_TOKEN" -H "Content-Type:application/json" -d '{"interface": {"port_id": "'$PORT_ID'"}}' | jq .
